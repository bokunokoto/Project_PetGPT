...
tab_schedule, tab_record, tab_medication = st.tabs(["📅 케어 일정", "🏥 진료 기록", "💊 약 복용"])
...
# ════════════════════════════════════════════════════════════════════
with tab_medication:
    st.subheader("💊 약 복용 관리")
    
    # 1. 복용 기록하기
    with st.container(border=True):
        med_pet_id, _ = pet_picker("대상 반려동물", "med_pet")
        med_name = st.text_input("약 이름", placeholder="예: 심장사상충 약, 항생제")
        
        if st.button("오늘 복용 완료", type="primary", key="med_done"):
            # 여기서 db.add_medication_log(pet_id, med_name, today) 함수를 
            # db.py에 새로 만들어서 호출하면 됩니다.
            st.success(f"{med_name} 복용 기록을 저장했습니다!")
            st.rerun()

    # 2. 복용 준수율 확인
    st.divider()
    st.subheader("📊 복용 준수율")
    st.info("최근 30일간 복용 기록을 기반으로 준수율을 보여줍니다.")
    # 향후 통계 그래프를 여기에 구현
    # 반려동물 필터
    filter_pet_id = None
    if pet_options:
